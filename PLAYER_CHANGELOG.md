...
- Refined World Event reward rows, improved text-field taps, and capped Android rendering for steadier performance on high-refresh devices.
...
- Reworked the Garden plot area into a consistent vertical scroll list with tighter bounds and clearer overflow behavior.
- Refined the Day theme with a warmer paper-textured room background and polished panel, progress, and action artwork.
- Improved Brewing ingredient-drop timing and simplified cauldron-switching motion.
...
- Compacted the Brewing room layout so cauldron details and actions fit more cleanly between the room header and chat.
...
- Refreshed Workshop side actions with clearer character and feature artwork.
- Polished the first-run account choice with a darker backdrop and smoother opening motion.

## 0.3.14

- Improved Garden planting actions with clearer capitalized, outlined labels.
...
- Improved stall cards and Elara's Market guidance to make loaded stock, sale progress, and the new allocation flow clearer.
...
- Kept the latest cauldron recipe, batch, and auto configuration safe when the game closes before the server finishes saving.
