...
Startup reconciliation replays that journal when it is ahead of the same save lineage, clears it when the server already contains that revision or a later one, and rejects it when another session advanced the server row. A pending save that lowers the hydrated server level is also stale and is discarded unless it records a new prestige completion, where the level reset is intentional. Explicit account-session takeover or reset invalidation may discard pending saves instead of replaying stale progress.

Initial own-save hydration has a bounded readiness deadline. If the subscription neither applies nor errors, startup reports `gameplay_save_timeout` and uses the normal connection retry flow instead of leaving the loading splash open indefinitely.
