...
It has `npm market` and `player market` tabs. `npm market` contains automatic NPC demand sales and shared NPC stock. `player market` contains local player requests and player-to-player listings.

The `npm demand market` block displays up to five automatic NPC sale stands. Stand 1 starts unlocked, the next locked stand shows a buy action, and later stands show `locked` until they become next. The NPC market does not repeat the top-panel gold total. Selected stands show the item and current sell price from the gameplay snapshot, while backend NPC need stays hidden there and only drives the underlying sale process. Selecting an unlocked stand opens a popup where the player chooses an exact item to auto-sell, grouped under `seeds`, `herbs`, and `potions` tabs, with the sell price beside each item. The popup also has an `empty` option to clear the stand.
