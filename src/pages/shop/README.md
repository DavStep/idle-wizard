...
The NPC market displays up to five stands. Stand 1 starts unlocked, the next locked stand shows a buy action, and later stands show `locked` until they become next. The NPC market does not repeat the top-panel gold total. Selected stands show the owned item quantity and current sell price from the gameplay snapshot. Selecting an unlocked stand opens a popup where the player chooses an exact item to auto-sell, grouped under `seeds`, `herbs`, and `potions` tabs, with live sell prices beside each item. The popup also has an `empty` option to clear the stand.
