...
} from './itemResearchStatus.js';
import { setItemIconLabel } from './itemIconLabel.js';
import { applyMysteryText } from './mysteryText.js';
import { setResourceColor } from './resourceColor.js';

const COLLAPSED_COLUMN_COUNT = 2;
const COLLAPSED_ROW_COUNT = 3;
const COLLAPSED_ITEM_COUNT = COLLAPSED_COLUMN_COUNT * COLLAPSED_ROW_COUNT;

export class RoomInventoryBoxManager {
  constructor({
    gameplayFacade,
    kind,
    title,
    ...
    rowClassName,
    dividerClassName,
    countClassName = 'room-inventory-box__count',
    toggleClassName = 'room-inventory-box__toggle',
    containerClassName = 'room-inventory-box',
    rowsBaseClassName = 'room-inventory-box__rows',
    rowBaseClassName = 'room-inventory-box__row',
    dividerBaseClassName = 'room-inventory-box__divider',
    getItems = () => [],
    filterItems = (snapshot, item) =>
    ...
    this.rowsBaseClassName = rowsBaseClassName;
    this.rowBaseClassName = rowBaseClassName;
    this.dividerBaseClassName = dividerBaseClassName;
  ...
  mount(parent) {
    ...
    this.root.className = `${this.containerClassName} ${this.rootClassName} style-box`;
    ...
    this.rows.className = `${this.rowsBaseClassName} ${this.rowsClassName}`;
  ...
  render(snapshot) {
    ...
    for (const item of items) {
      ...
      this.setRowIcon(refs.label, item);
  ...
  getVisibleRows(snapshot) {
    return this.getItems(snapshot)
      ...
      .filter((item) => this.filterItems(snapshot, item));
  }
  ...
  ensureRows(items) {
    for (const item of items) {
      ...
      row.className = `${this.rowBaseClassName} ${this.rowClassName}`;
  ...
  getDivider() {
    if (!this.divider) {
      ...
      this.divider.className = `${this.dividerBaseClassName} ${this.dividerClassName}`;
  ...
  renderToggle(itemCount) {
    ...
    const canToggle = itemCount > COLLAPSED_ITEM_COUNT;
  ...
  getVisibleCount(itemCount) {
    return this.expanded ? itemCount : Math.min(itemCount, COLLAPSED_ITEM_COUNT);
  }
