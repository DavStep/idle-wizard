...
describe('resource icon labels', () => {
  it('marks mana and gold words while preserving text content', () => {
    const element = document.createElement('span');

    setResourceIconText(element, 'cost 10 mana and 2 gold');

    expect(element.textContent).toBe('cost 10 mana and 2 gold');
    expect(element.querySelector('.style-resource-label--mana')).not.toBeNull();
    expect(element.querySelector('.style-resource-label--gold')).not.toBeNull();
    expect(
      element.querySelector('.style-resource-label--mana .style-resource-label__icon'),
    ).not.toBeNull();
    expect(
