...
Auto brewing stores enabled, armed, selected unlocked recipe, and auto-collect state independently for every cauldron. Auto-collect defaults off and can only be enabled while autobrew is enabled; when enabled it collects a ready batch so that the automation loop can continue. The save uses the numbered cauldron array, so additional cauldron slots inherit the same persistence behavior without slot-specific fields. Automation prepares that cauldron's recipe from inventory before starting the brew.
