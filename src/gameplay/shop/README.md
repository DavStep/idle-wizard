...
The first trader stand is a free starter stand and remains unlocked even when
an older hosted config or save reports zero unlocked slots. Later stands are
bought sequentially after their market-rank and player-level capacity becomes
available. Only the next stand can be purchased. Stands 2–5 cost `150`, `400`,
`1,000`, and `2,500` coin at the level 5, 10, 13, and 17 capacity milestones.
Existing saves keep every stand they already own.
Player listing and request capacity continues to follow the active market rank.
...
The visible Gems tab has two manual free offers. The coin offer grants current player level `* 20` coin, then enters a two-hour cooldown. The daily Amber offer uses the internal `dailyCrystalOffer` key, grants `1` Amber, then enters a 24-hour cooldown. Both cooldowns catch up offline, but neither reward is granted until the player collects it. Paid display offers include Amber bundles and matching-price Amethyst bundles at 100× the quantity; transactions remain unavailable.
