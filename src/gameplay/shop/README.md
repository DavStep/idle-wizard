...
Shared `market stock` also comes from the backend quote snapshot. Buying spends local gold, adds one local item, and calls the backend stock reducer first so all players pull from the same NPC stock.
