...
Each unlocked tile can hold one active crop and one selected seed. Planting consumes the selected seed, grows the matching herb for the herb definition growth time, then waits ready. Starting harvest runs a second timer; completion adds one herb, keeps the selected seed, and auto-replants it when another copy is owned. Selecting `empty` clears the next seed without clearing an active crop.
