...
Before making new UI, start with `docs/ui-patterns-index.md`, then read only the matching section or named entry in `docs/ui-patterns.md`. Reuse the nearest row, box, popup, tab, scroll pane, control, or border-label pattern instead of inventing a near-duplicate. Read the complete catalogue only for catalogue-wide audits, migrations, or when the router cannot identify a closest pattern.
...
UI editor parity is strict for new or changed editor coverage. Editor previews must instantiate the same exported production class or factory used by the game; editor-only replicas of game UI are forbidden. A new or materially changed scene, dialog, or `feature.*` preview must declare its reusable visual and interactive children in `childWidgetIds`, and every newly introduced or changed child must exist as an independently selectable `kind: 'widget'` UI Lab entry with a passive production-backed thumbnail. Do not turn an unrelated feature-local UI change into repository-wide legacy catalogue backfill; full completeness is a dedicated UI Lab migration or release-audit task. Fixtures may replace data and services, never rendering code.

For structural or visual UI, UX, layout, popup, dialog, screen, page, button, or flow changes, use `idle-wizard-ui-workflow`. Its risk tiers decide whether the focused consistency path is sufficient or the full `impeccable`, product/style, editor-parity, and browser-reference path is required.
...
- For multi-iteration UI work, if the shared server is down, start `npm run local:keepalive` once and reuse it across edits and screenshots. Do not repeatedly restart `npm run dev`; every cold start runs the full asset preparation chain.
