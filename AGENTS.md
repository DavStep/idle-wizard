...
UI editor parity is strict. Editor previews must instantiate the same exported production class or factory used by the game; editor-only replicas of game UI are forbidden. Every scene, dialog, or `feature.*` preview must declare all reusable visual and interactive children in `childWidgetIds`. Each declared child must exist as an independently selectable `kind: 'widget'` UI Lab entry with a passive production-backed thumbnail. If a large preview introduces or uses a reusable smaller component that is not independently catalogued, expose that production component and add its standalone entry in the same change. Fixtures may replace data and services, never rendering code.
