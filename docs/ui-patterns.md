...
- `RootRunSettingsTogglePixi` in `src/rendering/pixi/primitives/PixiSettingsControls.js` is the shared boolean switch matching the Root Run Settings dialog. It uses the same exact 23px knob asset as the settings slider, the Root Run brown capsule rail, and the lime inset for `on`. It supports `off`, `on`, pressed, and disabled states with immediate state changes and semantic activation. Feature rows own their label and locked copy; when a feature is locked, omit the toggle and show `locked` in the existing right-side status slot.
