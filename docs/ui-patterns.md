...
| Research station-upgrade row | `ResearchRowWidget` in `src/rendering/pixi/pages/research/ResearchPixiPage.js`; `.research-page__row` in `src/styles/base.css` | Compact `80px` research catalogue row using the shared Root Run station-upgrade card, a scaled left art well, a fixed middle description, a compact rank badge, and a scaled right cost/status control. Locked rows use the approved monochrome state; the title and description share the row ink color. |
...
| Cauldron Carousel | `BrewingHudPixi` in `src/rendering/pixi/pages/brewing/BrewingHudPixi.js` | Brewing station selector with one centered cauldron, previous/next arrows, horizontal swipe, and dynamic dots for unlocked cauldrons plus one next locked slot. At the five-cauldron cap no extra locked dot is rendered. |
