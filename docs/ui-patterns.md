...
| Research station-upgrade row | `ResearchRowWidget` in `src/rendering/pixi/pages/research/ResearchPixiPage.js`; `.research-page__row` in `src/styles/base.css` | Compact `80px` research catalogue row using the shared Root Run station-upgrade card, a scaled left art well, an adaptive middle description, a compact rank badge, and a scaled right cost/status control. The description uses the full fixed middle region and keeps the normal `11px` source size when possible, shrinking toward `8px` only when its wrapped height would overflow. Its measured vertical center includes a `-10px` optical correction so Lilita One's visible glyphs, rather than its low-sitting line box, center against the art and action. Locked rows use the approved monochrome state; the title and description share the row ink color. |
| Research Station Title Plaque | `ResearchStationTitlePlaque` in `src/rendering/pixi/pages/research/ResearchPixiPage.js`; `.research-page__box-title` in `src/styles/base.css` | Static Research category heading using the Root Run `header-red` horizontal nine-slice geometry with tab-owned color variants: Regular is yellow, Automation is red, Advanced is emerald green, and Crystal is purple. It sits flush on the content's left edge at `42px` source height, uses a single-line Title Case `18px` Lilita One label, and measures its width from the rendered label plus the tightened decorated right-cap allowance. Category rhythm is `18px` above the plaque and `5px` below it. Width caps at the Research content edge; only an overlong label may reduce toward the normal `13px` source size. The visible title still names the category, so tab color is not the only identifier. No selected, disabled, loading, error, empty, or motion variants. Approved Pixi evidence: `screenshots/research-title-variants-native-contact-sheet.png`, the four `screenshots/research-title-*-390x844.png` authored views, and `screenshots/research-title-variants-desktop-fitted.png`. |
