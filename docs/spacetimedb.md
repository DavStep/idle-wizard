...
- `player_maintenance`: private per-identity `drain` / `locked` support gates; clients can read only their own row through `own_player_maintenance`.
...
`admin_merge_player_accounts` moves one source account's progression and linked social/market/discovery rows into a target identity, preserves the target username, combines playtime, and deletes the source player. Both identities must be player-locked, or global maintenance must be locked. The source scoped-maintenance row is removed with the source player; support unlocks the surviving target after verification.

`admin_copy_player_progression` copies one player's gameplay save, player level, leaderboard income, and world-event leaderboard points onto another player identity without deleting the source player. It preserves the target player's username and visual profile fields, then kicks the target session so stale clients cannot immediately republish the old save. It does not copy chat, alliance membership, feedback, or player-market rows. The changed target identity must be player-locked, or global maintenance must be locked.

`admin_set_player_level_by_identity` sets one player's real gameplay level by identity. It updates `player.player_level`, `leaderboard.player_level`, world-event/alliance level snapshots, and gameplay save `tasks.currentLevel`, then kicks the target session so the next load hydrates the edited save. The target must be player-locked, or global maintenance must be locked.
...
Global maintenance mode lives in `game_config` under the `maintenance` key. Player-scoped maintenance lives in `player_maintenance` and is projected only to the matching connection through `own_player_maintenance`; the strictest global or player mode wins. `off` allows normal play. `drain` makes affected updated clients stop gameplay and flush one final `set_player_gameplay_save` while other writes from those identities are rejected. `locked` rejects all affected player writes, including gameplay saves. `admin_set_player_maintenance` changes only the named identity and closes that player's session when entering `locked`. Single-player admin mutations require either their affected identities to be player-locked or global maintenance to be locked. Use global `set_maintenance_mode` for bulk work and follow `docs/maintenance.md` before live data work. `migrate_player_gameplay_saves` requires global `locked`, preserves `savedAt`, never deletes rows, and records a one-time migration key in `maintenance_state`.
