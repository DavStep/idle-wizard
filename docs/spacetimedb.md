...
`sell_to_npc` records an NPC buyer sale. It reduces `npc_need`, raises the fulfilled/supply score, increases shared `npc_stock`, and recomputes the backend quote. `tick_npc_market` replenishes `npc_need` over time and recomputes prices from current need. `buy_from_npc` spends shared `npc_stock`, raises `npc_need`, and recomputes the quote. During a server data reset, the module clears NPC market demand and stock once using a `maintenance_state` key tied to `PLAYER_DATA_RESET_GUARD_MICROS`.
