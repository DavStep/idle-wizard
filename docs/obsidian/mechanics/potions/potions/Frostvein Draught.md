---
title: "Frostvein Draught"
tags:
  - mechanics
  - entity/potion
  - system/potions
status: active
world: mechanics
note_type: potion
system: potions
implementation: shipped
catalog_order: 27
entity_id: frostveinDraught
item_type_id: 2027
recipe_access: "global discovery + recipe research"
unlock_research_id: "unlockRecipe:frostveinDraught"
required_player_level: 26
previous_recipe_research: "[[mechanics/potions/potions/Frostmoss Cleanse|Frostmoss Cleanse]]"
base_sell_coin: 225.6
value_scope: effective-maincloud-items
mana_cost: 54
brew_seconds: 115
brew_time: "1m 55s"
ingredients:
  - "[[mechanics/garden/herbs/Frostmoss Herb|Frostmoss]]"
  - "[[mechanics/garden/herbs/Nettle Herb|Nettle]]"
  - "[[mechanics/garden/herbs/Mandrake Herb|Mandrake]]"
discovery: "[[mechanics/potions/Potion Discovery|Potion Discovery]]"
sold_through:
  - "[[mechanics/market/NPC Market|NPC Market]]"
  - "[[mechanics/market/Player Market|Player Market]]"
verified_on: 2026-08-21
source_files:
  - src/gameplay/items/managers/ItemDefinitionManager.js
  - src/gameplay/items/managers/PotionRecipeManager.js
  - src/gameplay/research/managers/ResearchDefinitionManager.js
---

# Frostvein Draught

> [!summary] At a glance
> Its exact ordered recipe is hidden from players until [[mechanics/potions/Potion Discovery|global discovery]]. Its effective item base sell value is **225.6 coin**.
> After discovery, other players unlock it through recipe research at level 26 after [[mechanics/potions/potions/Frostmoss Cleanse|Frostmoss Cleanse]].

## Ordered recipe

| Order | Herb | Quantity |
| ---: | --- | ---: |
| 1 | [[mechanics/garden/herbs/Frostmoss Herb|Frostmoss]] | 2 |
| 2 | [[mechanics/garden/herbs/Nettle Herb|Nettle]] | 1 |
| 3 | [[mechanics/garden/herbs/Mandrake Herb|Mandrake]] | 1 |

Brewing costs **54 mana** and takes **1m 55s** before bottling. Ingredient order matters.

## Access

| Property | Value |
| --- | --- |
| Access | Discovery owner, or recipe research after global discovery |
| Required player level | 26 |
| Previous recipe research | [[mechanics/potions/potions/Frostmoss Cleanse|Frostmoss Cleanse]] |
| Player visibility | hidden until discovered |

## Connections

- [[mechanics/garden/herbs/Frostmoss Herb|Frostmoss]] ×2
- [[mechanics/garden/herbs/Nettle Herb|Nettle]] ×1
- [[mechanics/garden/herbs/Mandrake Herb|Mandrake]] ×1
- Sold through: [[mechanics/market/NPC Market|NPC Market]] and [[mechanics/market/Player Market|Player Market]]

> [!note] Price meaning
> `base_sell_coin` is the item value. Actual market quotes can vary with demand and market licence.
