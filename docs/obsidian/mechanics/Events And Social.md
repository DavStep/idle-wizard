---
title: Events And Social
tags:
  - mechanics
  - event
  - social
status: active
world: mechanics
---

# Events And Social

## Personal Tasks

Personal tasks are daily quests owned by the client gameplay save. Completed
daily quests add points to daily and weekly reward tracks.

## Weekly World Events

World events give normal gameplay actions a world-news reason for the week.
Requests award contribution points; progress can complete a request, but
matching actions keep adding contribution points after completion.

## Guild

[[Guilds|Guild]] is a private player hall unlocked at level 15. Heavy state
stays in the gameplay save. Adventurers simulate in lazy 10-minute ticks.

## Trade Alliance

Trade alliances are player groups. Backend facades watch alliance tables, send
reducers, mirror chat, and grant crystal rewards from claimed weekly quests.

## Related

- [[Market Systems]]
- [[Prestige]]
- [[Guilds]]
