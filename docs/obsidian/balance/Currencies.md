---
title: "Currencies"
tags:
  - balance
  - currency
status: active
world: balance
note_type: balance
---

# Currencies

## Mana

Mana powers seed summoning and brewing. Player level sets max mana and mana per
second. Level 1 starts at 50 max mana and 1 mana per second; each level adds 50
max mana. Regen rises quickly at levels 2-5, slows at 6-10, then grows more
slowly from level 11 onward.

## Coin

Coin pays level completion costs, market purchases, capacity buys, and regular
research. It is earned through market flows and other normal loops.

## Crystal

Crystal is granted by player level, including level 1. It pays for plot and
cauldron multiplier levels and configured visual unlocks. It also appears in
shop support flows.

## Ruby

Ruby comes from Prestige milestones and pays for run-scoped automation such as
auto summon, planting, harvesting, brewing, and bottling. Current Ruby is
derived from completed Prestige milestones minus committed Ruby research.

## Emerald

Emerald persists through Prestige resets and pays for advanced studies such as
capacity, growth speed, research efficiency, stall staffing, and automation reserve.
Only explicitly permanent studies, such as capacity, survive as completed
research across Prestige.

## Related

- [[Level Curve]]
- [[Prestige Tuning]]
- [[Market Tuning]]
