...
- Post-reset replay guards must allow the client baseline save, including default free research like `unlockSeed:sageSeed` and starting coin with `current: 0, totalGenerated: 0`; server normalization must not reclassify later non-generated grants as leaderboard income.
...
- A SpacetimeDB token recovered from Android native storage during auth preparation is an existing app account, even when WebView `localStorage` was cleared by an update; do not keep the pre-prepare fresh-start decision after that recovery.
