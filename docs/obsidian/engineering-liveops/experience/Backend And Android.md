...
- Never retry a remembered SpacetimeDB identity anonymously after a transient connection failure; only explicit authentication rejection may rotate fallback tokens, and remembered Google accounts must fail closed instead of accepting a new anonymous token.
