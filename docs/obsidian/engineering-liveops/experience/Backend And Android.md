...
- Routine Google-auth reconnects must keep the durable stored SpacetimeDB account token as a fallback; suppress that fallback only while a pending account-link save proves the player is deliberately switching identities.
