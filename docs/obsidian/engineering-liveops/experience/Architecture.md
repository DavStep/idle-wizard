...
- FTUE placement harnesses should use memory-backed Elara storage; persisted drag placement or entry animations can mask auto-placement logic.
...
- Passive `.style-progress` bars share the compact 10px Root Rush source height; do not add per-use rail height overrides for timers or task bars. Interactive sliders and the top-panel quest rail use the shared 14px thickness tokens. Progress rails never act as scroll cues; scrollable views use the yellow right-edge station scrollbar. Apply semantic colors through page-scoped fill/edge tokens.
