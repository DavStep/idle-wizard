...
- Workshop side actions use the approved flat, rough-edged brown/tan/gold/purple family: plain intact Alliance flag, podium, checklist and quill, satchel, star ledger, sealed envelope, telescope, and calendar wand. Render each icon at `72%` inside its `50x50px` art frame, nudged `2px` toward the owned screen edge within the `50x60px` hit area, with `13.5px` outlined labels, a `-10px` icon-to-label offset, and a compact `62px` row pitch; do not add tint layers, mirroring, backing plates, or per-icon scales.
