...
- Idle Witch Craft splash art lives at `../idle-whitch-craft/core/assets/ui/mobile/splash-screen.png`; its loading gradient and art-relative placement live in `../idle-whitch-craft/core/splash.css`. Idle Wizard reuses that composition through `PixiOnlineGateView`.
...
- Room-background changes must update both the DOM `--style-page-background` rule and retained Pixi `PIXI_PAGE_BACKGROUND_COLORS`; production Pixi does not read the page CSS, and repeated Pixi stops should resolve to a solid fill instead of creating a zero-contrast gradient.
