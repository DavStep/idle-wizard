...
- Bottom room chrome is a shared five-tab panel (`brewing`, `garden`, `workshop`, `research`, `shop`); when gated `prestige` becomes visible, prepend it to the left of that default list. Use Root Run's full 224x294 room-tab active/inactive backgrounds as whole sprites, fill the source width, keep one baseline with an `8px` bottom safe gap, and only raise/enlarge/label the active tab. Unlocked icons stay full-color and full-opacity, active labels use white Title Case, and locked tabs replace the room icon with the shared larger lock centered lower in the inactive frame.
- Do not substitute Root Run's compact `bottom-building-popout-tools` tab caps for the full room-tab backgrounds. Render the room-tab artwork as one whole sprite/background so its lower curved panel art remains intact. Keep optional tabs in the same non-wrapping flex row so their frames cannot leak below the main baseline.
