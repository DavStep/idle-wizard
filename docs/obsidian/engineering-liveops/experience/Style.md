...
- UI editor nine-slice conversion is metadata-only: preserve the selected source PNG byte-for-byte, write its validated `.9slice.json` beside it, and let the production source manifest rediscover that sidecar on reload.
