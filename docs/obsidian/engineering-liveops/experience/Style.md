...
- Workshop side actions use the approved flat, rough-edged brown/tan/gold/purple family: plain intact Alliance flag, podium, checklist and quill, satchel, star ledger, sealed envelope, telescope, and calendar wand. Render each icon at `79.2%` inside its `50x50px` art frame within the unchanged `50x72px` hit area, with `16px` outlined labels, a `-4px` icon-to-label offset, and a compact `74px` row pitch; do not add tint layers, mirroring, backing plates, or per-icon scales.
...
- Load Stall, Workshop Bag, and Workshop Stats keep their category tabs fully inside the brown shell footer with `4px` above and `6px` below; never align tab bottoms flush to the shell edge. Expand gaps by count (`4px` for five, `6px` for four, `8px` for three) so shorter rows breathe. Load Stall uses split paper; Bag and Stats keep one continuous paper board.
...
- Standard retained dialog inner boards and image-backed row frames keep Account's `298px` visible width. Reserve the shared scrollbar gutter by shifting the full-width row slightly left, extending the viewport `2px` beyond the row before the shared track gap, and keeping a `3px` paper inset beyond the track; never narrow the row to make the scrollbar fit.
