...
- Workshop side-action icons use Bag-matched contain-fit sizing, thick near-black contours, upper-left light, broad lower-right shadow, saturated flat color, and no material texture. `Tasks` uses `assets/game/source/icons/icon-personal-tasks-scroll-bag-style.png`; `Event` uses `assets/game/source/icons/icon-quests-scroll-bag-style.png` while the event dialog keeps the guild secretary portrait.
