...
- Keep loading progress honest across startup phases: asset and OTA work may use measured determinate progress, while authentication and server connection use an indeterminate range with explicit phase or issue copy. Never replace unknown connection work with a scripted percentage.
...
- Guild bottom destinations use the shared icon-backed room-tab contract: Hall banner/key, Adventurers shield/sword, Fishers fish/hook, Miners pickaxe/crystal, and World globe. Locked future destinations still replace their icon with the shared lock.
...
- FTUE prerequisites must be reachable at the level where they are taught. The level-2 sale requires one free unlocked NPC stand from level 1; enforce that floor after remote shop config and save hydration so an older `initialUnlockedSlots: 0` row cannot relock it.
