...
- Shared passive progress rails use Root Run's exact compact capsule assets: the 10px source-height `progress-bar-bg.9.png` track and alpha-masked `progress-bar-fill.9.png` silhouette, with only their centers stretched. Retained scrollbars rotate those same capsule assets vertically so both thumb caps remain authored and round. Purple is the default fill, Brewing uses blue, Garden uses green, and Market/Research use yellow. Trader-stall sale progress is the Market exception: keep it purple and do not inherit the gradient player style. The top-panel level-up rail reuses Root Run's exact panel, track, and fill alpha silhouette while retaining the approved Idle Wizard gradient; app-level connection/loading rails keep their selected gradient treatment. Interactive sliders and the top-panel quest rail use the thicker 14px height.
