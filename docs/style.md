...
- The Workshop `Summon Seed` action composes the shared purple regular-button nine-slice with the stacked cost-button content layout while enabled, then swaps to the shared gray regular-button nine-slice while unavailable. Both the action and cost row use the shared black text stroke, and the second row uses the shared mana-drop icon with a light numeric amount.
