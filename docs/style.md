...
- The Workshop `Summon Seed` action composes the shared purple radius-30 regular-button nine-slice with the stacked cost-button content layout while enabled, then swaps to the matching gray radius-30 skin while unavailable. The smaller tier keeps its protected highlight and corners clear of the stacked label at the authored `92x52` size. Both the action and cost row use the shared black text stroke, and the second row uses the shared mana-drop icon with a light numeric amount.
