...
- Generated scene and icon art defaults to neutral daylight white balance unless the feature explicitly calls for night. Keep warm wood and gold local to those materials; do not wash the whole image in amber, orange, sepia, candlelight, dusk, or sunset grading. Repeated art collections should include clear-day, bright-cloud, or similarly readable daytime variations instead of defaulting every piece to a dark night scene.
...
- Missing-resource counts and unaffordable cost values use the shared bright
  insufficient red `#ff5965`; both projections must stay identical.
...
- Standard inner sections (`style-box`) use the shared Root Run shop-tile nine-slice, recolored per selectable theme. Preserve its qUIck slice margins at the Root Run `390 / 1080` design scale and keep the theme's exact center-sample color as the rounded underlay; the underlay only closes fractional raster gaps and must not replace or square off the transparent frame corners. The persistent top player panel uses a dedicated `30px`-corner sibling generated from the inactive bottom-room-tab nine-slice with the vertical flip baked into the PNG. Stretch it from the screen top across the full source width, with a `16px` gap before room content, and do not flip it again at runtime. Keep outer room landmarks, dialogs, controls, tutorial surfaces, and feature-owned art on their dedicated approved chrome.
- Do not add standalone CSS rounding to image-backed panels; the shared skin owns its silhouette, bevel, inset, and corners.
- Solid squircle fills reuse the white nine-slice family in `assets/game/source/ui/white-squircle` and apply semantic color at runtime with Pixi tint or a CSS mask fill. Keep the source textures white and do not add color-baked duplicates. The checked-in PNGs have their Android marker border stripped; their adjacent metadata owns the source slice margins.
...
- Every player-facing popup/dialog panel uses the Root Run Expedition composition: `expedition-dialog-back.9.png` as the brown outer nine-slice, `expedition-dialog-front.9.png` as the paper content nine-slice, shared `banner-purple.9.png` as the centered purple title plaque, and `expedition-dialog-close.png` as the round dismiss control centered below the shell. Render the assets through `PixiDialogFrame` in mirrored room UI, with CSS border-image/image fallbacks for non-mirrored dialogs.
...
- Every managed scroll panel uses the single `style-page-scroll` primitive, including room pages, dialogs, popups, and bounded lists. It owns the shared station-panel model: wheel and pointer drag share progressive asymmetric edge resistance, release inertia with `2.4` damping, elastic top/bottom limits, and the exact `520`/`26` station spring return. Release velocity includes the final pointer-up position in both DOM and retained paths. Reduced motion stops release inertia and clamps directly to the nearest valid bound. The thumb compresses against either overscrolled edge. Preserve the `6.5px` final-width vertical track with `4.333333px` final block inset, translucent `#17100c` fill, and 72%-alpha black outline; the proportional thumb uses `#f2ae54`, a `#5e321b` outline, and a `29.611111px` final minimum height. Retained Pixi divides the Root Run geometry by its `3x` source layer exactly once; DOM fallback geometry is authored directly at the final `390px` surface. Keep the scrollbar on the right edge of the actual viewport, visible only when content overflows. Do not append a horizontal progress rail below scroll content.
- Tooltips use the active theme's ordinary image-backed panel frame, the shared generated pointer aimed at the invoking control, and a small offset gray shadow. Never show the generic solid panel fallback. Outside presses dismiss the tooltip; opening uses one short reduced-motion-safe pointer-anchored snap.
...
- The Workshop `Summon Seed` action composes the shared purple radius-50 regular-button nine-slice with the stacked cost-button content layout while enabled, then swaps to the matching gray radius-50 skin while unavailable. Both the action and cost row use the shared black text stroke, and the second row uses the shared mana-drop icon with the selected-batch amount. A yellow compact `xN` control beside it cycles the researched batch range, and the ritual sign uses the shared star-rank label to show the Summon Seed upgrade.
...
- Brown/gold is the default action family, green is positive/claim/collect/confirm, and red is cancellation or loss. Cost buttons compose the regular-button color family with resource content; popup tabs reuse its brown, dark-brown, and gray textures.
- Regular button chrome lives in `assets/game/source/ui/regular-button` as a complete blue, brown, dark-brown, gray, green, purple, red, and yellow matrix. Each color owns radius tiers `50`, `30`, and `15`; those tiers have minimum source sizes `141x171`, `87x104`, and `46x53` with source slices `L86 T100 R52 B68`, `L52 T60 R32 B41`, and `L27 T30 R16 B20`. Every tier includes the bottom shadow, an explicit flat `3x3` scalable center, and one matching flat gutter pixel around each center edge. Keep decoration and curved-cap pixels outside those gutters so smoothed stretching cannot extend a highlight or break an edge. Use the smallest tier whose corner geometry fits the authored control height. Cost buttons and popup tabs reuse this matrix; popup tabs change only selected/resting color while inheriting the base button's fitted output geometry.
- Room-level text tabs and compact room actions use the shared `36px` source height. This includes Research and Market page tabs, Garden `Plant All`, `Harvest All`, and `Seeds`, and Brewing `Recipes`, `Auto`, and `xN`. Dialog footer tabs keep their separate `28px` widget contract and use `13px` top plus `9px` bottom output insets over the shared regular-button textures, leaving a live center band at the shorter height.
- Treat every nine-slice as a complete skin binding: texture, source slices, logical output insets, and minimum center. Its minimum rendered size comes from opposing output insets plus that minimum center. The UI editor, qUIck, and retained Pixi setters must reject an undersized binding before changing the active skin; never silently compress protected borders to make it fit.
- Shared passive progress rails use losslessly cleaned Root Run capsule assets: the one-pixel Android metadata border is stripped from `progress-bar-bg.9.png` and `progress-bar-fill.9.png`, then only their clean centers stretch at the 10px source height. Retained scrollbars are a separate contract: match Root Run's station component with antialiased Pixi `Graphics.roundRect` track and thumb shapes, exact scaled geometry, and no progress-rail asset reuse. Purple is the default progress fill, Brewing and Research use blue, Garden uses green, and Market uses yellow. Trader-stall sale progress is the Market exception: keep it purple and do not inherit the gradient player style. The top-panel level-up rail keeps Root Run's exact track and fill alpha masks at the authored 51px qUIck height, applies the fixed black and yellow tints at runtime, and uses no Idle Wizard gradient; its flat panel backing reuses the shared white-squircle family. App-level connection/loading rails keep their selected gradient treatment. Interactive sliders and the top-panel quest rail use the thicker 14px height.
...
- Workshop level-up is the authored celebration exception: hand off the visible HUD star into a one-shot snake glide to center at `4x`, shake/pop into the new numbered star, return it to the HUD, show then clear the `Level Up!` ribbon, and reveal large-icon rewards and unlocks serially. Do not run this ceremony outside Workshop or overlap its stages.
...
- Shared top chrome keeps three equal slots in every room: Coin on the left, the active room currency in the center, and Bag on the right. Workshop and Brewing use Mana as `current/cap` with the blue regeneration rate beneath it; other rooms use their Amber, Ruby, or Emerald context currency without a rate. Amethyst stays available inside Bag instead of occupying top chrome. Keep Settings at its original size. The panel ends at source y `88`, then preserves the existing `16px` gap before room content at y `104`.
