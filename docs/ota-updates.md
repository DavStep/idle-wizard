...
bundles are published with GitHub Pages. After the game becomes playable, a
compatible update opens a mandatory in-game prompt. The player starts the
download explicitly, sees real downloaded and total megabytes on the loading
splash, and the app safely restarts into the finished bundle.
...
- Before activation, the app saves and flushes current progress. It then uses
  the native updater's immediate `set` operation, which restarts the JavaScript
  context into the downloaded bundle.
